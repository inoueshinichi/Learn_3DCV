...
from BasicModule.coordinate_state import (
    CoordinateState,
    CoorRH_PYup_PZforward_NXright_State, # OpenGL系
    CoorRH_PZup_PYforward_PXright_State, # OpenCV系
    CoorLH_PYup_PZforward_PXright_State,  # DirectX系
    CoorLH_PZup_NYforward_PXright_State,  # UnrealEngine系
)
...
from Homography.homography import find_homography2D_with_ransac
...
def pose_from_view(V: np.ndarray, 
                   geo_cxt: GeometryContext
    ...
    # 各軸のベクトル
    forward = geo_cxt.forward_axis(rot)
    right = geo_cxt.right_axis(rot)
    up = geo_cxt.up_axis(rot)
...
def compute_camera_with_plane_marker_homography2D(
        K: np.ndarray,
        template_marker_planar_pts: np.ndarray,
        target_marker_planar_pts: np.ndarray,
        ) -> np.ndarray:
    """平面マーカーと2Dホモグラフィを用いたカメラのPose(位置, 姿勢)推定

    Args:
        K (np.ndarray): キャリブレーション行列[3x3]
        template_marker_planar_pts (np.ndarray): 真正面に平面マーカーが映ったテンプレート画像から得られた特徴点 (3xN)
        target_marker_planar_pts (np.ndarray): カメラのPose推定を行いたい平面マーカーが映った画像から得られた特徴点 (3xN)
    """

    # 2Dホモグラフィ行列を推定
    H = find_homography2D_with_ransac(planar1_pts=template_marker_planar_pts, # 第一平面の2D点群[3xN] (x,y,w)
                                      planar2_pts=target_marker_planar_pts)   # 第二平面の2D点群[3xN] (x,y,w)
    
    # 真正面に平面マーカーが映ったテンプレート画像に対するカメラ行列
    V1 = np.zeros((3,4), dtype=np.float32) # (R|t)
    V1[:3,:3] = np.eye(3,3) # 単位行列 I -> (I|t)
    V1[:3,3] = np.array([0,0,-1], np.float32) # t = (0,0,-1) 平面マーカーに対して上方向に`1`の位置にカメラ中心を置く
    P1 = K @ V1 # (3,4)

    # 2Dホモグラフィ行列Hを用いてカメラのPose推定を行いたい平面マーカーが映った画像に対応するカメラ行列を計算
    P2 = H @ P1 # HP (3,4)

    # 2Dホモグラフィの推定は左手系(画像左上を原点に↓をY軸正, →をX軸正, ↑をZ軸正)を元に推定しているので, 
    # 左手座標系仕様になっている. カメラの右手座標系(OpenCV系:原点に↓をY軸正, →をX軸正, ↓をZ軸正)に
    # 戻すため(もしくは, その可能性があるため)に, 反転してるZ軸(基底)をクロス積で新しく作る.
    # 平面マーカー上のZ軸の向きは不明. なので, P2の第1列, 第2列, 第4列のみ正しい. 
    # Z軸の向きを設定する第3列は, 第1列と第2列のクロス積で作る必要がある.

    A = np.linalg.inv(K) @ P2[:,:3]
    axis_x = A[:,0] # (3,1)
    axis_y = A[:,1] # (3,1)
    axis_z = np.cross(axis_x, axis_y) # (3,1) クロス積 新規作成
    R = np.array([axis_x, axis_y, axis_z], dtype=np.float32).T # 転置
    P2[:,:3] = K @ R
